...
$$
Q(\Theta \lvert \Theta_{t}) = \E_{Z \lvert X, \Theta_{t}}[\log p(X, Z \lvert \Theta)]
$$

This definition of the $Q(\Theta \lvert \Theta_{t})$ involves calculating the expected value of the log-likelihood of both the observed and latent data, conditioned on the observed data $X$ and the current estimate of the parameters $\Theta_{t}$. Essentially, this step assesses how well the current model parameters explain the observed data and predicts the latent variable assignments.
...
The M-step involves optimizing the  $Q(\Theta \lvert \Theta_{t})$ to determine $\Theta_{t+1}$, with the constraint that the mixture probabilities $\pi_1, \dots, \pi_K$ sum to one. This optimization is achieved using the method of Lagrange multipliers, which allows for the incorporation of the equality constraint directly into the maximization process. We formulate the Lagrangian as follows:
